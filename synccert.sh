...
DNS_NAME=$(docker run -e TS_SOCKET=/var/run/tailscale/tailscaled.sock -e TS_STATE_DIR=/var/lib/tailscale -v tailscale_run:/var/run/tailscale -v /docker/appdata/state:/var/lib/tailscale:rw -v /etc/ssl:/etc/ssl:rw tailscale/tailscale:latest tailscale status --json | jq -r .Self.DNSName | sed -e's/\.$//' | tr -d '\n')
